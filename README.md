...
A phone-first NFL pick’em app with the complete official 2026 schedule, configurable light and dark themes, all 32 team logos, required favorite-team profiles with the selected team shown in the header avatar, private leagues, one-tap picks that toggle off before kickoff, sportsbook-implied win chances, league pick percentages, server-enforced kickoff locks, automatic NFL results, an Outrights tab with logo-based division and Super Bowl winner predictions, a commissioner-selected lock week, and a configurable Super Bowl season-points award, weekly/monthly/season standings, a dedicated league-picks tab with green/red results for correct, incorrect, and missed selections, and commissioner controls. Each game’s picks become visible to the league at kickoff. Published selections backed by 20% or less of the league are labeled as wild picks in the table. A selection changed after publication is labeled Upset while it remains the league’s only pick for that team. Commissioners can enable or disable each standings badge for their league. Standings include Perfect Week achievements and leader badges for Lone Wolf, Upset King, No Guts No Glory, public wild picks, picks against a favorite team (Vende Patrias), incorrect picks on a favorite team (Titanic Musician), and missed games (Mama Pichas) for the selected week, month, or season; tied leaders share each leader badge. Correct Super Bowl predictions earn the Nostradamus badge without changing points. Standings badges appear as icons, with a matching responsive legend below the standings that includes only enabled badges currently earned by league members. Weekly member progress is visible to the full league while anyone remains incomplete. Commissioners can publish a snapshot containing only matchups for which every league member has submitted a pick. When every member completes the full week, the progress panel disappears and all weekly picks become public automatically.
